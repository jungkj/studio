...
export const SeedEssaysButton: React.FC = () => {
  const [isSeeding, setIsSeeding] = useState(false);
  const { toast } = useToast();

  const handleSeedEssays = async () => {
    setIsSeeding(true);
    let successCount = 0;
    let errorCount = 0;
    ...
    try {
      ...
      if (successCount > 0) {
        toast({
          title: "Essays Added!",
          description: `Successfully added ${successCount} example essays.${errorCount > 0 ? ` ${errorCount} failed.` : ''}`,
        });
      } else {
        toast({
          title: "Failed to Add Essays",
          description: "No essays were added. Check if they already exist or if you're authenticated.",
          variant: "destructive",
        });
      }
    } catch (error) {
      console.error('Error seeding essays:', error);
      toast({
        title: "Error",
        description: "An error occurred while adding essays.",
        variant: "destructive",
      });
    } finally {
      setIsSeeding(false);
