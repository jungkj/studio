...
const Index = () => {
  ...
  const desktopIcons = [
    { name: 'myComputer', label: 'My Computer', icon: 'my_computer.png' },
    { name: 'work', label: 'My Work', icon: 'briefcase.png' },
    { name: 'essays', label: 'My Essays', icon: 'documents.png' },
    { name: 'gamesLauncher', label: 'Games', icon: 'game_controller.png' },
    { name: 'terminal', label: 'Terminal', icon: 'console.png' },
    { name: 'contact', label: 'Contact', icon: 'mail.png' },
    { name: 'about', label: 'About Me', icon: 'user.png' },
  ];

  return (
    <div className="w-screen h-screen bg-black flex items-center justify-center p-4">
      <div className="w-full h-full aspect-[4/3] mac-desktop-bg flex flex-col relative overflow-hidden shadow-2xl">
        ...
        <div className="flex-grow relative">
          ...
          {windowStates.welcome.isOpen && (
            <Window title="Welcome" onClose={() => closeWindow('welcome')} initialPosition={{ x: 100, y: 80 }} zIndex={windowStates.welcome.zIndex} onFocus={() => focusWindow('welcome')}>
              <WelcomeWindow onClose={() => closeWindow('welcome')} />
            </Window>
          )}
          {windowStates.essays.isOpen && (
            <Window title="My Essays" onClose={() => closeWindow('essays')} initialPosition={{ x: 50, y: 50 }} zIndex={windowStates.essays.zIndex} onFocus={() => focusWindow('essays')}>
              <EssaysApp onClose={() => closeWindow('essays')} />
            </Window>
          )}
          {windowStates.about.isOpen && (
            <Window title="About Me" onClose={() => closeWindow('about')} initialPosition={{ x: 150, y: 100 }} zIndex={windowStates.about.zIndex} onFocus={() => focusWindow('about')}>
              <AboutApp onClose={() => closeWindow('about')} />
            </Window>
          )}
          {windowStates.myComputer.isOpen && (
            <Window title="My Computer" onClose={() => closeWindow('myComputer')} initialPosition={{ x: 250, y: 150 }} zIndex={windowStates.myComputer.zIndex} onFocus={() => focusWindow('myComputer')}>
              <MyComputerApp onClose={() => closeWindow('myComputer')} />
            </Window>
          )}
          {windowStates.gamesLauncher.isOpen && (
            <Window title="Games Launcher" onClose={() => closeWindow('gamesLauncher')} initialPosition={{ x: 350, y: 200 }} zIndex={windowStates.gamesLauncher.zIndex} onFocus={() => focusWindow('gamesLauncher')}>
          ...
          {windowStates.ticTacToe.isOpen && (
            <Window title="Tic-Tac-Toe" onClose={() => closeWindow('ticTacToe')} initialPosition={{ x: 400, y: 250 }} zIndex={windowStates.ticTacToe.zIndex} onFocus={() => focusWindow('ticTacToe')}>
              <TicTacToe />
            </Window>
          )}
          {windowStates.snake.isOpen && (
            <Window title="Snake" onClose={() => closeWindow('snake')} initialPosition={{ x: 450, y: 300 }} zIndex={windowStates.snake.zIndex} onFocus={() => focusWindow('snake')}>
              <SnakeGame onClose={() => closeWindow('snake')} />
            </Window>
          )}
          {windowStates.sudoku.isOpen && (
            <Window title="Sudoku" onClose={() => closeWindow('sudoku')} initialPosition={{ x: 500, y: 350 }} zIndex={windowStates.sudoku.zIndex} onFocus={() => focusWindow('sudoku')}>
              <SudokuGame onClose={() => closeWindow('sudoku')} />
            </Window>
          )}
          {windowStates.solitaire.isOpen && (
            <Window title="Solitaire" onClose={() => closeWindow('solitaire')} initialPosition={{ x: 550, y: 400 }} zIndex={windowStates.solitaire.zIndex} onFocus={() => focusWindow('solitaire')}>
              <SolitaireGame onClose={() => closeWindow('solitaire')} />
            </Window>
          )}
          {windowStates.contact.isOpen && (
            <Window title="Contact Me" onClose={() => closeWindow('contact')} initialPosition={{ x: 600, y: 450 }} zIndex={windowStates.contact.zIndex} onFocus={() => focusWindow('contact')}>
              <ContactApp onClose={() => closeWindow('contact')} />
            </Window>
          )}
          {windowStates.work.isOpen && (
            <Window title="My Work" onClose={() => closeWindow('work')} initialPosition={{ x: 650, y: 500 }} zIndex={windowStates.work.zIndex} onFocus={() => focusWindow('work')}>
              <WorkApp onClose={() => closeWindow('work')} />
            </Window>
          )}
          {windowStates.terminal.isOpen && (
            <Window title="Terminal" onClose={() => closeWindow('terminal')} initialPosition={{ x: 700, y: 550 }} zIndex={windowStates.terminal.zIndex} onFocus={() => focusWindow('terminal')}>
